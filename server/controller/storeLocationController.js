...
export const getStores = async (req, res) => {
    ...
    try {
        ...
        const store = await Store.find({})
            .select('name address postalcode country state status') // Limit fields
            .populate('address', 'name') // 👈 populate only _id and name of category
            .skip(skip)
            .limit(limit)
            .sort({ createdAt: -1 });
        ...
        res.status(200).json({
            ...
            store, // 👈 category will be like: { _id, name }
        });
    } catch (err) {
        res.status(500).json({ message: 'Server Error', error: err.message });
    }
